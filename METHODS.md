...
**A preliminary, separate note.** Beiran & Litwin-Kumar (2025) develop a method for ranking which neurons are most informative to record from, given a connectome. Adapting that method here, greedy selection of cell types that best reconstruct the full-population representational geometry, gives an early result on the ON-only condition: as few as 4 of 65 cell types recover this geometry at Spearman r ≥ 0.8, dominated by T4 subtypes. This is a single run on one stimulus condition, without permutation testing, replication across conditions, or comparison against a proper null distribution, and should be read as a preliminary sign this direction is tractable, not a validated finding. It does not bear on the real-vs-random wiring question above; it only uses the real, connectome-constrained models.
...
**A related, deeper assumption, now tested.** The Henning reference pools recordings across 14 flies, distinct from the specific individual whose reconstructed connectome underlies Flyvis, so every biology-mediated comparison in this document implicitly assumes pooled, cross-individual recordings are an adequate stand-in for recordings from the modeled individual itself. Using the raw data's per-fly identity, a leave-one-fly-out test directly checks this: each individual fly's own reference correlated against the reference pooled from the other 13. Mean Spearman r = 0.81 (raw reconstruction) / 0.77 (von Mises), against a matched random split-half baseline (recomputed on this same data) of r = 0.99 / 0.98, a real gap of 0.18–0.22. Individual flies vary considerably in how well they match the pool, per-fly r ranged from 0.47 to 0.94, meaning some individuals are markedly less typical than others. Independently reproduced on a second machine with matching results. Honest reading: cross-individual substitution is meaningfully, not marginally, less reliable than within-pool splitting, though still well above chance. This is a real, quantified limitation on the biology-mediated comparisons throughout this document, not just an untestable assumption anymore.
