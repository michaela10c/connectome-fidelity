...
- [Preliminary Explorations Prompted by External Review](#preliminary-explorations-prompted-by-external-review)
- [Answer to the Brunton Question](#answer-to-the-brunton-question)

## Fly Connectome

To test this, I used Flyvis (Lappalainen et al. 2024), a connectome-constrained model of the *Drosophila* visual system. Its architecture is built directly from the reconstructed fly connectome: which cell types connect to which, and through which edges, is fixed by the reconstruction itself. What training optimizes is the strength of those connections, the synaptic weights, via gradient descent on an optic-flow estimation task, using Flyvis's own built-in training entry point rather than a custom training loop, so nothing about the recipe itself diverges from how the connectome-constrained ensemble was originally trained. In short: a decoder learns to read the network's motion signal, and every part of that training setup is held fixed except which connectome the network started from. Concretely, the readout is a dedicated decoder (`DecoderGAVP`, shape [8, 2], kernel size 5), trained against an L2-norm flow loss plus an activity penalty, optimized with Adam under a stepwise learning-rate schedule (5×10⁻⁵ down to 5×10⁻⁶) over 250,000 iterations, batch size 4. Everything about this setup, the decoder, the loss, the optimizer, the stimulus batch, is held identical across every condition; only the connectome adjacency itself, real or random, differs. Comparing real wiring against random alternatives, matched in scale but not structure, and training each the same way, gives a direct test of what the connectome's structure itself contributes, independent of what training does to the weights sitting on top of it.

"The fly work" actually spans two different scopes, worth keeping distinct. The simulation itself covers the full fly visual system, the whole optic lobe circuitry (retina, lamina, medulla, lobula, and lobula plate), 65 distinct cell types in total, every population vector and RDM throughout this document is built from all 65, not a filtered subset. T4/T5 specifically enter only at the biological-validation stage (items 2 and 3): they're the one cell type pair real, recorded direction-tuning data actually exists for (Maisak et al. 2013; Henning et al. 2022), so comparisons against biology are necessarily restricted to T4/T5. That's a limitation of the available biological reference, not of the simulation, which covers all 65 cell types throughout.

Representational geometry, following Kriegeskorte et al. 2008, captures this at the population level: present a set of stimuli, record the population response to each, and compare the full pattern of pairwise distances between responses rather than any single unit's activity. If connectome-constrained wiring produces a distinct geometry that random wiring cannot replicate, that geometry is a candidate fidelity signal, measurable without any behavioral readout at all. Conceptual work has begun surveying the broader landscape of possible fidelity tests for whole-brain emulation (Linssen & Koene 2025), including network-dynamics measures and standardized behavioral test batteries, but without proposing and empirically validating a specific candidate metric against connectome-constrained models; this project contributes one such candidate, tested directly.
...
A follow-up asked whether the connectome alone, no trained activity, predicts this same ranking: comparing the activity-based selection against purely structural rankings (in-degree, out-degree, and synapse-weighted versions of each, computed directly from the connectome). In-degree correlates weakly with the activity-based ranking (Spearman r = 0.41–0.51 across two independent runs, p < 0.001); out-degree does not reach significance (r = 0.23, p = 0.07); top-10 overlap between structural and activity-based rankings is poor throughout (1–4 of 10 shared); and the T4 subtypes dominating the activity-based ranking do not appear among the highest-degree cell types by any structural measure tested. Independently reproduced on a second machine with closely matching numbers. Honest reading: connectome degree alone only partially predicts informativeness, and this too is a single-condition, preliminary result, not yet checked against the other two stimulus conditions or validated to the standard of the rest of this document.
...
**What "circular structure" means, and why it's a problem here.** The stimulus set for this whole project is a set of directions arranged around a compass, 0° to 360°, wrapping back on itself. Purely from that layout, without any biology involved at all, two directions that are close together on the compass (say 10° and 20°) will look more similar than two directions that are far apart (say 10° and 190°), just because of how angles work. That predictable, purely geometric pattern, built entirely from angular position on the circle, is what "circular structure" refers to.

The reason this matters: almost *any* reasonable model, biologically accurate or not, real wiring or random, will trivially reproduce that pattern, because it's a property of the stimulus set's layout, not of the specific computation any given network performs. If a biological reference is built in a way that's dominated by this same circular pattern, correlating a model's geometry against that reference mostly measures whether the model respects basic angular ordering, something nearly every model does automatically, rather than whether it captures the specific, non-trivial shape of real biological tuning. A reference like that can't actually discriminate a faithful model from an unfaithful one, both would score well, for the wrong reason. That's the confound this item is checking for directly.
...
Two null schemes were tested: a degree-preserving swap, a Maslov-Sneppen edge swap that keeps each cell type's in- and out-degree, its number of distinct connection partners, exactly fixed while scrambling which specific partners those are (at the swap intensity used, roughly 39% of connection pairs actually changed on the real connectome), and Erdős–Rényi, which draws entirely new random connection pairs with no degree constraint at all, the more aggressive of the two schemes. Both preserve total edge count, total synapse count, and Dale's-law sign structure; only the specific pattern of who connects to whom differs, and only degree-preserving swap keeps the number of distinct partners per cell type intact. Both were trained through the complete 250,000-iteration recipe, then evaluated against both Henning references, the same way as CC.
...
One direction this resampling can't rule out, worth naming rather than leaving implicit: downsampling CC tests whether the reported effect was real, not whether a smaller real effect exists that n=10 simply lacks the power to detect. Erdős-Rényi's resampled p-values ranged from 0.04 to 0.21 across draws, wider than degree-preserving swap's, consistent with limited power at this sample size, not necessarily a hidden effect, but not fully ruled out either. Closing this required training the null schemes up, not resampling CC down. Wiring files already existed for both schemes up to n=25, so both were extended: degree-preserving swap trained cleanly to n=25; Erdős–Rényi's extension reached n=25 but one network was excluded as unstable (a constant, non-direction-selective output, all pairwise RDM distances exactly zero), leaving n=24. Evaluated directly against real wiring (n=50) with no resampling needed, the extended Erdős–Rényi comparison remains non-significant (raw reference, p = 0.49), consistent with the resampled analysis's null interpretation, not an underpowered real effect that a larger sample would reveal.
...
**Table 6.** Fraction of networks trending negative against the von Mises reference, trained vs. untrained populations. "% negative" is the fraction of models in each population whose own single correlation against the reference is itself negative, a snapshot measure, not a within-network trend. Degree-preserving swap and Erdős-Rényi reflect updated evaluations at n=25 and n=24 respectively (one Erdős-Rényi network excluded as unstable); the pooled row is recomputed at these updated sample sizes.

| Population | Trained? | n | % negative | Significance |
|---|---|---|---|---|
| CC | Yes | 50 | 78% | p = 0.0001 |
| Erdős-Rényi | Yes | 24 | 67% | not significant (p = 0.152) |
| Degree-preserving swap | Yes | 25 | 76% | p = 0.0146 |
| Pooled trained (both null schemes) | Yes | 49 | 71% | p = 0.0038 |
| Weight-shuffled random | No | 50 | 54% | not significant (chance) |

The cleanest single comparison here is CC vs. weight-shuffled random, matched in size (n=50 each) and significantly different on the raw reference (Mann-Whitney p = 0.008). The two trained-random null schemes now point in opposite directions at their updated sample sizes: degree-preserving swap reaches significance alone (p = 0.0146) where it did not at n=10, while Erdős-Rényi, significant alone at the original n=10 (100% negative, p = 0.002), loses significance at n=24 (67% negative, p = 0.152). Pooled across both schemes at their current sample sizes (n=49), the combined result remains significant (71% negative, p = 0.0038).
...
Given the pooled comparison's own size mismatch (n=20 vs. n=50), the same resampling check already applied to item 3's Erdős-Rényi result was run here too: the untrained population was repeatedly subsampled down to n=20 (10,000 draws, no new training needed) and the comparison rerun on each draw. Unlike item 3's original result, this one held up robustly: median p = 0.0077 across all draws (IQR 0.0028-0.0193), with 92.7% of size-matched draws reaching p < 0.05. The pooled trained-vs-untrained finding was never actually riding on the size mismatch. This resampling check was performed at the pooled group's original size (n=20); at the updated pooled size (n=49), the two groups are already naturally near-matched to the untrained n=50, and the reported p = 0.0038 above no longer needs a resampling correction to be read as a fair comparison.
...
This is now a genuinely solid finding, not a partial one: the matched CC-vs-untrained comparison was already clean on its own, and the null-scheme evidence, once fairly tested, generalizes it robustly beyond CC specifically, not resting on the mismatched sample the way it initially appeared to. It also connects back to, and helps resolve, item 3's Erdős-Rényi ambiguity directly: at the extended n=24 sample, Erdős-Rényi's own snapshot result reverses from significant (100% negative, p = 0.002 at n=10) to not significant (67% negative, p = 0.152), agreeing with item 3's own extended, non-resampled Method A result (p = 0.49) that the original apparent signal was a small-sample artifact rather than a robust effect of wiring identity.
...
## Preliminary Explorations Prompted by External Review

Following circulation of an earlier draft, two questions raised in correspondence prompted three follow-up analyses, summarized in the table below. None of these has undergone the validation applied to the five items above: each is a single run on a single stimulus condition (or, for the cross-individual test, the one dataset available), without permutation testing, replication across conditions, or comparison against a proper null distribution. They are reported here only as preliminary indications that these directions are tractable, together with their headline numbers, not as validated findings on par with items 1-5.

| Question | Method | Result |
|---|---|---|
| Which cell types are most informative for reconstructing the trained population's representational geometry? | Greedy selection maximizing reconstruction of the full-population RDM | 4 of 65 cell types reach Spearman r ≥ 0.8; dominated by T4 subtypes |
| Does connectome structure alone (no activity) predict this same ranking? | In-/out-degree, computed directly from the connectome, compared against the activity-based ranking above | In-degree r = 0.41–0.51 (p < 0.001); out-degree r = 0.23 (p = 0.07, n.s.); T4 subtypes not among highest-degree cell types |
| Can cross-individual (donor) recordings substitute for within-individual recordings in the Henning reference? | Leave-one-fly-out (14 flies) vs. matched random split-half, recomputed on the same raw data | Leave-one-out r = 0.81 (raw) / 0.77 (von Mises) vs. split-half r = 0.99 / 0.98; gap of 0.18–0.22; per-fly r ranged 0.47–0.94 |

The first two rows were independently reproduced on a second machine with closely matching numbers; the third was reproduced with matching results to three decimal places. Read together, they suggest three things: this class of follow-up question is empirically tractable with the existing pipeline; connectome degree alone is a weak proxy for which units matter most; and the pooled, cross-individual biological reference used throughout this document substitutes for within-individual data meaningfully, but not perfectly, a real, quantified caveat on the biology-mediated comparisons in items 2 and 3, not merely a theoretical one.
