...
**Update: the von Mises and raw-data references' divergence in the table above is not just lower power on the raw side — it traces to a specific, identified reconstruction artifact in the von Mises version, not a flaw in the raw-data version.** This surfaced in a separate leave-one-cell-type-group-out robustness check on the earlier CC-vs-random comparison (against the raw-data reference): the raw-data version's per-model result did not cleanly replicate the von Mises version's specific finding on that same check, and tracing the discrepancy pointed to the von Mises curve-fitting step itself, not to noise or to a weakness in the raw, no-curve-fitting extraction. **Practical implication: the raw-data reference is the more trustworthy of the two wherever the two disagree, and any result reported here that depends heavily on the von Mises column specifically should be read with that in mind** — this applies retroactively to every `r_von_mises` value in this document and in the training-trajectory work, not just to the table above. The raw reference remains lower-powered (n=8 stimuli, no curve-fitting to sharpen the signal), so this isn't a case for discarding von Mises outright, but for treating agreement between the two as the trustworthy case and divergence as a signal to trust raw over von Mises specifically, not an ambiguous tie.
