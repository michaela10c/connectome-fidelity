...
**Correction to an earlier version of this note: the claim that the divergence "traces to a specific, identified reconstruction artifact" was an overstatement not supported by any diagnostic work actually done, and is retracted.** What's actually established, no more and no less: a robustness check (`check_reconstruction_robustness.py`) confirmed the von Mises-based CC-vs-random finding holds up under *within-framework* parametric perturbation (a different curve shape, kappa scaled 0.5x/2x) — that's a real, positive robustness result for von Mises on its own terms. Separately, rebuilding the reference directly from raw per-direction data with no curve-fitting at all did not replicate the finding — neither CC nor random reached significance, and the ordering flipped. **No diagnostic work has actually compared the two reconstructions against each other** — no per-cell check of whether von Mises fits systematically over- or under-estimate tuning width, no check for convergence failures on a subset of cells, nothing isolating a mechanism. The honest state: the finding didn't survive the move from a parametric to a non-parametric reconstruction, and why is genuinely unknown. **Neither reference should be treated as more authoritative than the other until that diagnostic is actually run** — a real bias toward preferring raw as the lower-assumption method is a defensible general instinct, but raw is also the lower-powered version (n=8 stimuli, no curve-fitting to sharpen a weak signal), so a null result there doesn't confirm von Mises is wrong; it's equally consistent with raw lacking the power to detect a real effect. This applies to every `r_von_mises` and `r_raw` pair reported in this document and in the training-trajectory work — read disagreements between them as an open question, not as raw winning by default.
