...
- **Untrained real wiring is clearly distinguishable from random wiring** by representational geometry, for every null scheme and stimulus condition tested ([Table 0](METHODS.md#1-can-geometry-tell-real-wiring-from-random-wiring)).
...
- **Convergence with training is abrupt, not gradual**: nearly the entire shift happens within the first few percent of a network's training run, then plateaus for the rest ([Figure 6](METHODS.md#3-does-geometry-distinguish-real-wiring-from-trained-random-wiring)).
