...
**Separate from the validated findings above:** three preliminary follow-up analyses, prompted by questions raised in outside correspondence, are summarized in a table in **[METHODS.md](METHODS.md#preliminary-explorations-prompted-by-external-review)**. None has been through the same validation as the findings above; each is a single run, not yet permutation-tested or replicated across conditions. Headline results: a small subset of cell types (4 of 65, dominated by T4 subtypes) can reconstruct most of the full-population representational geometry; connectome degree alone only weakly predicts which cell types those are; and cross-individual (donor) recordings substitute for within-individual data in the Henning reference meaningfully, but not perfectly (a real, quantified gap, not just a theoretical caveat).

## Full Methodology & Results

The complete, item-by-item methodology, tables, and figures behind the findings above live in **[METHODS.md](METHODS.md)**: five experiments building on each other (real-vs-random wiring untrained and trained, the biological-reference validation, the training-vs-wiring analysis, and the retraining-based instability test), a set of preliminary follow-up analyses prompted by outside correspondence, and the full answer to the Brunton question. Throughout this README, "item N" refers to the correspondingly numbered section in METHODS.md, linked below wherever it's mentioned.
